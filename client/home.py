...
class HomeScreen(object):
    """
    Creates the GUI for the home screen
    """
    ...
    def log_out(self):
        """
        Handles loggin the user out when closing the window
        """
    ...
    def draw_gui(self):
        """
        Creates, sets up and displays the Home Screen GUI
        """
    ...
    def new_chat_window(self, *args):
        """
        Shows a pop-up dialog for starting a new chat
        """
    ...
    def error_window(self, err):
        """
        Shows an error dialog if a user that was entered in the "start new chat" window does not exist

        Parameters:
            err (string): The error returned by the server to be displayed
        """
        top= Toplevel(self.root)
        top.geometry("350x150+300+300")
        # set location
    ...
    def update_unreads(self, chat_id):
        """
        Obsolete function to show the number of unread messages for a chat
        """
    ...
    def close_win(self, top, entry):
        """
        Creates a new chat when the new chat dialog is closed

        Parameters:
            top (TK object): the dialog this was called from
            entry (Entry): the text box that the list of new users was entered into
        """
